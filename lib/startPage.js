...
exports.login = async function(req, res) {
    ...
    try {
        ...
        if (result.length !== 0) {
            ...
            res.status(200).json({ result: 'success', id: id, token: token });
        }
        else {
            res.status(401).json({ result: 'fail', message: 'ID 또는 PW가 틀렸습니다.' });
        }
    }
    catch (error) {
...
exports.signUp = async function(req, res) {
    ...
    try {
        ...
        if (result.length > 0) {
            return res.status(409).json({ result: 'fail', message: '중복된 ID입니다.' });
        }
        ...
        return res.status(500).json({ result: 'fail', message: '서버 오류가 발생했습니다', error : error });
    }
    finally {
        if (conn) {
            await conn.end(); // 연결 종료
...
exports.checkNickName = async function(req, res) {
    ...
    try {
        ...
        if (result.length > 0) {
            return res.status(409).json({ result: 'fail', message: '중복된 닉네임입니다.' });
        } else {
            return res.status(200).json({ result: 'success', message: '사용 가능한 닉네임입니다.' });
        }
    }
    catch (error) {
        return res.status(500).json({ result: 'fail', message: '쿼리 오류' });
    }
    finally {
        if (conn) {
            await conn.end(); // 연결 종료
