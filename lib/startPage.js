...
exports.signUp = async function(req, res) {
    ...
    try {
        ...
        //console.log(starting_poketmon);
 
        // 회원가입 - book Table에 5마리 스타팅 포켓몬 저장. poke_id = idx
        sql = `INSERT INTO book (bakjoon_id, poke_id, poke_Lv, poke_Exp, poke_title) VALUES (?, ?, ?, ?, ?)`;
        for (const poketmon of starting_poketmon) {
            const poke_id = poketmon.idx;
            const poke_title = poketmon.poke_title;
            await conn.execute(sql, [id, poke_id, 1, 0, poke_title]);
          }
