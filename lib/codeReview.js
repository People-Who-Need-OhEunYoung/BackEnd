...
//             let masterSql = `SELECT nickName FROM reviewer WHERE review_id=? AND is_master=1`;
//             const [masterResult] = await conn.execute(masterSql, [review_id]);
//             let masterNickName = masterResult.length > 0 ? masterResult[0].nickName : null;
//             return { review_id: review_id, master: masterNickName };
//         });
...
//         if (search) {
//             sql += ` WHERE problem_title LIKE ? OR problem_no LIKE ? OR room_title LIKE ?`;
//         }

//         console.log(`실행된 SQL: ${sql} with ['%${search}%', '%${search}%', '%${search}%']`);


//         const [reviews] = search 
//             ? await conn.execute(sql, [`%${search}%`, `%${search}%`, `%${search}%`]) 
...
//         // 필요한 필드 조회
//         let sql = `SELECT review_id, room_title, problem_no, problem_title, problem_tier, max_peope, room_owner FROM review`;
...
//         const masterQueries = reviewIds.map(async review_id => {
//             let masterSql = `SELECT bakjoon_id FROM reviewer WHERE room_id=? AND room_owner=1`;
//             const [masterResult] = await conn.execute(masterSql, [review_id]);
//             let masterNickName = masterResult.length > 0 ? masterResult[0].nickName : null;
//             return { review_id: review_id, master: masterNickName };
//         });
...
exports.reviewList = async function(req, res) {
    ...
    db.query(query, (error, results) => {
        if (error) {
            console.error('데이터를 못가져왔음:', error);
            res.status(500).send('서버에러임');
            return;
        }
    ...
    try {
        ...
        try {
            ...
            for(let i =0; i < reviewer.length; i++) {
                ...
                await conn.execute(sql, [get_exp, get_credit, reviewer[i].bakjoon_id]);
            }

            await conn.commit();

            return res.json({ result : 'success', message : '참여자에게 경험치, 크레딧 제공 완료', user : reviewer, get_exp : get_exp, get_credit : get_credit});
        }
