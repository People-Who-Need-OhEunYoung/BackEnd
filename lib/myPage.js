...
exports.changeMonster = async function(req, res) {
    ...
    try {
        ...
        if (bookRows.length === 0) {
            return res.status(404).json({ result: 'fail', message: '소유하지 않은 포켓몬' });
        }
        ...
        // 성공 응답 반환
        return res.status(200).json({ result: 'success', message: '포켓몬 변경 완료' });
    }
    catch (error) {
        console.error(error);
        return res.status(500).json({ result: 'fail', message: '오류가 발생했습니다.' });
...
exports.changeNickName = async function(req, res) {
    ...
    try {
        ...
        // 성공 응답 반환
        return res.status(200).json({ result: 'success', message: '닉네임 수정 완료' });
    }
    catch (error) {
        console.error(error);
        return res.status(500).json({ result: 'fail', message: '오류가 발생했습니다.' });
...
exports.resolvedProblems = async function(req, res) {
    ...
    try {
        ...
        if(result.length === 0) {
            return res.status(404).json({result : 'fail', message : '해결한 문제가 없습니다.'})
        }

        console.log(result[0].resolved_date);
        result[0].resolved_date = result[0].resolved_date.toISOString().split('T')[0];

        return res.status(200).json({result : 'success', resolvedProblems : result});
