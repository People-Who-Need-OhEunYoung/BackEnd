        ...
        if(!codeCompare.includes('채점할 수 없습니다.')) {
            ...
            if(testCase) {
                if(testCase.length > 0) {
                    ...
                    // 코드 비교 스크립트 실행
                    const { stdout : codeCompare2, stderr } = await execPromise(`./codeCompare2.sh ${decoded.id}_${bojNumber} ${encodedTestCaseJson}`);
                    ...
                    if(codeCompare.includes('축하합니다')) {
                        ...
                        if(isCorrect === 0) {
                            ...
                            try {
                                ...
                                sql =`UPDATE resolved SET resolved_date=CURDATE(), elapsed_time=?, limit_time=?, success=?, get_credit, get_Exp WHERE bakjoon_id=? AND problem_id=?`;
                                await conn.execute(sql, [elapsed_time, limit_time, 1, get_credit, get_Exp, decoded.id, bojNumber]);
                                ...
                                console.log(`codeCompare2 135 : ${codeCompare2}`);
