...
exports.runCode = async function(req, res) {
    ...
    try {
        ...
        await execPromise(`chmod codeCompare.sh +x`);
        ...
        if(!codeCompare.includes('채점할 수 없습니다.')) {
            ...
            if(testCase) {
                if(testCase.length > 0) {
                    ...
                    await execPromise(`chmod codeCompare2.sh +x`);
                    ...
                    if(codeCompare.includes('축하합니다')) {
                        ...
                        sql =`UPDATE resolved SET resolved_date=CURDATE(), elapsed_time=?, limit_time=?, success=?`;
                        await conn.execute(sql, [elapsed_time, limit_time, 1]);

                        return res.json({ result: 'success', data: codeCompare, data2 : codeCompare2, isCorrect : 1});
                    }
                    // 테케있음/오답 응답 반환
        ...
        if(codeCompare.includes('축하합니다')) {
            ...
            return res.json({ result: 'success', data: codeCompare, isCorrect : 1 });
        }
