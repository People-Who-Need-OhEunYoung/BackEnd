...
exports.runCode = async function(req, res) {
    ...
    try {
        ...
        if(correct1) {
            ...
            const [correct] = await conn.execute(sql, [req.user.id, bojNumber]);
            ...
            if(isCorrect === 0) {
                ...
                try {
                    ...
                    await conn.execute(sql, [elapsed_time, limit_time, 1, get_exp, type, req.user.id, bojNumber]);
                    ...
                    if(type2 === '') {
                        ...
                        const [legend1] = await conn.execute(sql, [req.user.id]);
                        const leg1 = legend1[0][type1]; // 해당타입 경험치

                        sql = `UPDATE exp_by_type SET ${type1}=${type1}+? WHERE bakjoon_id=?`;
                        await conn.execute(sql, [get_exp, req.user.id]);

                        // 오른 후 경험치 확인
                        sql = `SELECT ${type1} FROM exp_by_type WHERE bakjoon_id=?`;
                        const [legend2] = await conn.execute(sql, [req.user.id]);
                        ...
                        const evo = await evolution1(req.user.id, leg1, leg2, parts, conn);
                        if(evo) {
                            evolutionResults.push(evo);
                        }
                        
                        // 레벨 100 첫 달성 시 전설의 포켓몬 지급
                        if(
                        leg1 < 10000 && leg2 >= 10000 || 
                        leg1 < 11000 && leg2 >= 11000 ||
                        leg1 < 12000 && leg2 >= 12000 ||
                        leg1 < 13000 && leg2 >= 13000) {
                            sql = `UPDATE user SET ${coinType}=${coinType}+1 WHERE bakjoon_id=?`;
                            await conn.execute(sql, [req.user.id]);
                            legendResults.push(`축하합니다!! ${parts[0]}유형 전설의 포켓몬 코인을 획득했습니다!!`);
                        }  
                    }
                    // 유형 2개일 때
                    else {
                        ...
                        await conn.execute(sql, [get_exp, get_exp, req.user.id]);
                        ...
                        const evo = await evolution2(req.user.id, leg1, leg2, leg11, leg22, parts, conn);
                        if(evo) {
                            evolutionResults.push(evo);
                        }
                        ...
                        // 레벨 100 첫 달성 시 전설의 포켓몬 지급
                        if(
                        leg11 < 10000 && leg22 >= 10000 || 
                        leg11 < 11000 && leg22 >= 11000 ||
                        leg11 < 12000 && leg22 >= 12000 ||
                        leg11 < 13000 && leg22 >= 13000) {
                            sql = `UPDATE user SET ${coinType2}=${coinType2}+1 WHERE bakjoon_id=?`;
                            await conn.execute(sql, [req.user.id]);
                            legendResults.push(`축하합니다!! ${parts[1]}유형 전설의 포켓몬 코인을 획득했습니다!!`);
                        }
                    }
                    await conn.commit();
                    ...
                    if(evolutionResults.length > 0) {
                        message += `포켓몬 ${evolutionResults.length}마리가 진화했습니다!! `;
                    }
                    if(legendResults.length > 0) {
                        message += legendResults.join(' ');
                    }
                    if(message === '') {
                        message = '처음 해결한 문제입니다. 유형별 경험치를 지급합니다.';
                    }
                    ...
                    return res.status(201).json({ 
                        ...
                        message: message, 
                        isCorrect: 1, 
                        evolutionPoketmon: evolutionResults.length > 0 ? evolutionResults : undefined,
                        legendPoketmon: legendResults.length > 0 ? parts.filter((_, i) => legendResults[i]) : undefined,
                        type1: `${type1}+${type2 ? get_exp : get_exp}`,
                        type2: type2 ? `${type2}+${get_exp}` : undefined
                    });
                }
                catch(transactionError) {
                    await conn.rollback();
                    console.error(`트랜잭션 오류 : ${transactionError}`);
                    return res.status(500).json({ result : 'fail', data: `${transactionError.message}`});
                }
            }
            // 이미 해결한 문제일 경우 경험치/크레딧 제공 안함
            // 일반 이미 정답
            return res.status(200).json({ result: 'success', message : '이미 해결한 문제입니다.', isCorrect : 1, get_exp : get_exp});
        }
        // 테케있음/오답 응답 반환
        // 일반 오답
        return res.status(200).json({ result: 'success', message : '오답입니다!!', isCorrect : 0});
    }
    catch (error) {
        console.error(`오류 발생: ${error}`);
        return res.status(500).json({ result: 'fail', data: `${error.message}` });
    }
    finally {
        if(conn) {
...
exports.selectOrUpdateResolvedProblem = async function(req, res) {
    ...
    try {
        ...
        const [result] = await conn.execute(sql, [req.user.id, problem_id]);
        ...
        if(result.length === 0) {
            ...
            if(limit_time) {
                ...
                await conn.execute(sql, [req.user.id, problem_id, 0, limit_time, 0, problem_title, type]);

                return res.status(201).json({result : 'success', message : '문제에 처음 도전했습니다. resolved 테이블에 limit_time을 포함한 데이터 추가'})
            }
            ...
            // limit_time 없으면 추가 안함
                ...
                await conn.execute(sql, [req.user.id, problem_id, 0, 0, problem_title, type]);

                return res.status(201).json({result : 'success', message : '문제에 처음 도전했습니다. resolved 테이블에 데이터 추가'})
            }
        }
        ...
        if(limit_time) {
            ...
            await conn.execute(sql, [limit_time, req.user.id, problem_id]);

            sql = `SELECT elapsed_time, limit_time FROM resolved WHERE bakjoon_id=? AND problem_id=?`;
            const [result2] = await conn.execute(sql, [req.user.id, problem_id]);

            return res.status(200).json({result : 'success', message : '기존에 풀던 문제, 제한시간 업데이트 성공', data : result2});

        }
        else {
            // resolved 데이터 있었을 경우 바로 리턴
            return res.status(200).json({result : 'success', message : '기존에 풀던 문제입니다. 데이터를 가져옵니다.', data : result});
        }
    }
    catch(error) {
        console.error(error);
...
exports.setTime = async function(req, res) {
    ...
    try {
        ...
        await conn.execute(sql, [elapsed_time, limit_time, req.user.id, problem_id]);

        return res.status(200).json({result : 'success', message : '타이머 업데이트 성공'})
