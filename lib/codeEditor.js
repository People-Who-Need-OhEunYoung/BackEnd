...
exports.runCode = async function(req, res) {
    ...
    try {
        ...
        const correct = req.body.correct; // true OR false
                            ...
                            try {
                                ...
                                if(flag === 1) {
                                    ...
                                    if(evolutionflag === "1") {
                                        ...
                                        return res.json({ result: 'success', message : `포켓몬 1마리가 진화했습니다!!`, data: codeCompare, data2 : codeCompare2, isCorrect : 1, evolutionPoketmon : evo});
                                        
                                    }
                                    else if(evolutionflag === "2") {
                                        return res.json({ result: 'success', message : `포켓몬 2마리가 진화했습니다!!`, data: codeCompare, data2 : codeCompare2, isCorrect : 1, evolutionPoketmon : evo});
                                    }
                                    else {
                                        if(getLegend1 !== '') {
                                            if(getLegend2 !== '') {
                                                return res.json({ result: 'success', message : `${getLegend1}, ${getLegend2}`, data: codeCompare, data2 : codeCompare2, isCorrect : 1, type1 :`${type1}+${get_exp}`, type2 : `${type2}+${get_exp}`, legendPoketmon1 : `${parts[0]}`, legendPoketmon2 : `${parts[1]}`});
                                            }
                                            else {
                                                return res.json({ result: 'success', message : `${getLegend1}`, data: codeCompare, data2 : codeCompare2, isCorrect : 1, type1 :`${type1}+${get_exp}`, type2 : `${type2}+${get_exp}`, legendPoketmon1 : `${parts[0]}`});
                                            }
                                        }
                                        else if(getLegend2 !== '') {
                                            return res.json({ result: 'success', message : `${getLegend2}`, data: codeCompare, data2 : codeCompare2, isCorrect : 1, type1 :`${type1}+${get_exp}`, type2 : `${type2}+${get_exp}`, legendPoketmon1 : `${parts[1]}`});
                                        }
                                ...
                                // 유형 1개일 경우
                                    ...
                                    if(evolutionflag === "1") {
                                        ...
                                        if(getLegend1 !== '') {
                                            return res.json({ result: 'success', message : `${getLegend1}`, data: codeCompare, data2 : codeCompare2, isCorrect : 1, type1 :`${type1}+${get_exp}`, legendPoketmon1 : `${parts[0]}`});    
                                        }
                                        // 일반 정답
            ...
            if(isCorrect === 0) {
                ...
                try {
                    ...
                    if(flag === 1) {
                        ...
                        // 1개만 진화했을 경우
                        if(evolutionflag === "1") {
                            return res.json({ result: 'success', message : `포켓몬 1마리가 진화했습니다!!`, data: codeCompare, isCorrect : 1, evolutionPoketmon : evo});
                            
                        }
                        else if(evolutionflag === "2") {
                            return res.json({ result: 'success', message : `포켓몬 2마리가 진화했습니다!!`, data: codeCompare, isCorrect : 1, evolutionPoketmon1 : evo, evolutionPoketmon2 : evo});
                        }
                        else {
                            if(getLegend1 !== '') {
                                if(getLegend2 !== '') {
                                    return res.json({ result: 'success', message : `${getLegend1}, ${getLegend2}`, data: codeCompare, isCorrect : 1, type1 :`${type1}+${get_exp}`, type2 : `${type2}+${get_exp}`, legendPoketmon1 : `${parts[0]}`, legendPoketmon2 : `${parts[1]}`});
                                }
                                else {
                                    return res.json({ result: 'success', message : `${getLegend1}`, data: codeCompare, isCorrect : 1, type1 :`${type1}+${get_exp}`, type2 : `${type2}+${get_exp}`, legendPoketmon1 : `${parts[0]}`});
                                }
                            }
                            else if(getLegend2 !== '') {
                                return res.json({ result: 'success', message : `${getLegend2}`, data: codeCompare, isCorrect : 1, type1 :`${type1}+${get_exp}`, type2 : `${type2}+${get_exp}`, legendPoketmon2 : `${parts[0]}`});
                            }
                    ...
                    // 유형 1개일 경우
                        ...
                        if(evolutionflag === "1") {
                            ...
                            if(getLegend1 !== '') {
                                return res.json({ result: 'success', message : `${getLegend1}`, data: codeCompare, isCorrect : 1, type1 :`${type1}+${get_exp}`, legendPoketmon1 : `${parts[0]}`});    
                            }
