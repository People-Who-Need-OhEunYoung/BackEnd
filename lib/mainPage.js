...
exports.headerData = async function(req, res) {
    ...
    try {
        ...
        const row = result[0];
        return res.json({ result: 'success', nickName: row.nick_name, credit: row.credit, curPokeId: row.cur_poke_id, user_id : decoded.id });
    }
    catch (error) {
...
exports.mainPageHeaderData = async function(req, res) {
    ...
    try {
        ...
        const userSql = `SELECT nick_name, cur_poke_id, credit, user_exp FROM user WHERE bakjoon_id=?`;
...
exports.legendGambling = async function(req, res) {
    ...
    try {
        ...
        if(coin === 'math_coin') {
            ...
            let sql2 = `SELECT * FROM book WHERE bakjoon_id=? AND poke_type=?`;
            const [legend] = await conn.execute(sql2, [decoded.id, '수학']);
            if(legend.length < 3) {
            ...
            // 전포를 다 가지고 있지 않다면 뽑기 진행
            let sql2 = `SELECT * FROM book WHERE bakjoon_id=? AND poke_type=?`;
            const [legend] = await conn.execute(sql2, [decoded.id, '자료구조']);
            if(legend.length < 3) {
            ...
            // 전포를 다 가지고 있지 않다면 뽑기 진행
            let sql2 = `SELECT * FROM book WHERE bakjoon_id=? AND poke_type=?`;
            const [legend] = await conn.execute(sql2, [decoded.id, '구현']);
            if(legend.length < 3) {
            ...
            // 전포를 다 가지고 있지 않다면 뽑기 진행
            let sql2 = `SELECT * FROM book WHERE bakjoon_id=? AND poke_type=?`;
            const [legend] = await conn.execute(sql2, [decoded.id, '그래프']);
            if(legend.length < 3) {
            ...
            // 전포를 다 가지고 있지 않다면 뽑기 진행
            let sql2 = `SELECT * FROM book WHERE bakjoon_id=? AND poke_type=?`;
            const [legend] = await conn.execute(sql2, [decoded.id, 'DP']);
            if(legend.length < 3) {
                ...
                // 이거 완성한// 이거 완성한 다음에// 이거 완성한 다음에// 이거 완성한 다음에
                // 코인 소모하는 로직도 추가해야함// 코인 소모하는 로직도 추가해야함// 코인 소모하는 로직도 추가해야함

                

                return res.json({ result : 'fail', message : `이미 해당 유형 포켓몬을 `})
            }
        }
        ...
        // 전포 뽑기 성공 메시지
        return res.json({ result : 'success', message: '전설의 포켓몬 뽑기에 성공했습니다!!!', legendPoketmon : `${leg.poke_type} : ${leg.poke_name}`});
    }
    catch(error) {
        console.error(error);
        return res.json({ result: 'fail', message: '오류가 발생했습니다.' });
