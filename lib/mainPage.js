...
exports.evolution = async function(req, res) {
    ...
    try {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
        await conn.execute(sql, [decoded.id, pok_id, result[0].poke_title, result[0].poke_eval, result[0].poke_legend_yn, result[0].poke_type, result[0].poke_name, result[0].poke_img, result[0].poke_profile_img]);

        // 현재 포켓몬으로 변경
        sql = `UPDATE user SET cur_poke_id=? WHERE bakjoon_id=?`;
        await conn.execute(sql, [pok_id, decoded.id]);

        return res.json({ result: 'success', poke_id: pok_id, message: '도감 등록 성공' });
    }
    catch (error) {
        console.error(error);
        return res.json({ result: 'fail', message: '오류가 발생했습니다.' });
